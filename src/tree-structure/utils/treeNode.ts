...
const cloneObject = (a: any, options: any): any => (a);
...
export function deepMerge(chunkA: TreeNode | undefined, chunkB: TreeNode | undefined) {
    ...
    const child: ChunksList<TreeNode> = getChildren(lastCutNodeA)
        .concat(getChildren(firstCutNodeB));
...
export function iterateThroughTree(
    nodes: Array<any>,
    callback: (item: any, parent?: any) => void,
    parent?: any
): void {
    nodes.forEach((node) => {
